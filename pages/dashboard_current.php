...
$currentMonthEnd = $currentMonthStart->modify('first day of next month');
$monthlyTrend = dashboard_empty_current_month_trend($currentMonthStart, $currentMonthEnd);
$yearlyTrend = dashboard_empty_year_trend();
...
if ($dbReady && $pdo !== null) {
    ...
    for ($month = 1; $month <= 12; $month++) {
        if (isset($trendRows[$month])) {
            dashboard_apply_sales_trend_row($yearlyTrend[$month - 1], $trendRows[$month], $canViewProductCost);
        }
    }
    ...
    $monthlyStatement->execute([
        ...
        ':month_end' => $currentMonthEnd->format('Y-m-d 00:00:00'),
    ]);

    foreach ($monthlyStatement->fetchAll() as $row) {
        $monthlyRows[(string) $row['sale_day']] = $row;
    }

    foreach ($monthlyTrend as $index => $day) {
        if (isset($monthlyRows[$day['date']])) {
            dashboard_apply_sales_trend_row($monthlyTrend[$index], $monthlyRows[$day['date']], $canViewProductCost);
        }
    }

    if ($canViewProductCost) {
        dashboard_apply_net_adjustments($pdo, $yearlyTrend, 'monthly', $currentYear);
        dashboard_apply_net_adjustments($pdo, $monthlyTrend, 'daily', $currentMonthStart->format('Y-m-d 00:00:00'), $currentMonthEnd->format('Y-m-d 00:00:00'));
...
$trendBadge = match ($trendMode) {
    ...
    'yearly' => format_money($trendTotal) . ' this year',
    default => format_money($trendTotal) . ' this month',
};
$chartAxisStep = $trendMode === 'monthly' ? 4 : 1;
...
function dashboard_empty_year_trend(): array
{
    $trend = [];
...
function dashboard_empty_current_month_trend(DateTimeImmutable $monthStart, DateTimeImmutable $monthEnd): array
{
    $trend = [];
    $dayIndex = 0;
    ...
    for ($day = $monthStart; $day < $monthEnd; $day = $day->modify('+1 day')) {
        $trend[] = [
            'date' => $day->format('Y-m-d'),
            'label' => $dayIndex === 0 ? $day->format('M j') : $day->format('j'),
            'tooltip_date' => $day->format('M j'),
            'revenue' => 0.0,
            'sold_cost' => 0.0,
            'gross_profit' => 0.0,
