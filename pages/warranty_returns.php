...
<div class="page-heading">
    ...
    <a class="top-action" href="<?php echo e(app_url('?page=recent-returns')); ?>">
...
<section class="warranty-return-layout">
    <article class="panel warranty-service-panel" id="warranty-return-form">
            ...
            <form class="service-wizard" method="post" action="<?php echo e(app_url('actions/warranty_return_save.php')); ?>" data-service-form data-service-lookup-url="<?php echo e(app_url('actions/warranty_return_lookup.php')); ?>">
                ...
                <section class="service-step is-active">
                    ...
                    <div class="service-step-body">
                        <label class="field product-picker service-invoice-search" data-service-search-field>
                            <span>Customer or invoice</span>
                            <input type="search" placeholder="Search by customer, phone, email, or invoice number" autocomplete="off" data-service-search>
                            <div class="product-suggestions service-search-results" data-service-suggestions hidden></div>
                        </label>

                        <div class="service-selected-invoice" data-service-selected-invoice hidden>
                            <div>
                                <strong data-service-selected-invoice-title>Selected invoice</strong>
                                <span data-service-selected-invoice-meta></span>
                            </div>
                            <button class="service-change-invoice" type="button" data-service-change-invoice>
                                <i data-lucide="repeat-2"></i>
                                Change
                            </button>
                        </div>

                        <div class="service-items-table-wrap" data-service-items-wrap hidden>
                            <table class="service-items-table">
                                ...
                                <tbody data-service-items>
                                    <tr><td colspan="5" class="service-items-empty">Select an invoice to view items.</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                        ...
                        <div class="warranty-form service-details-form">
                            ...
                            <div class="field" data-service-refund-fields data-service-refund-method-field>
                                ...
                                <select name="refund_method">
                                    ...
                                    <option value="none">No payment / refund</option>
                                </select>
                            </div>
    ...
    <article class="panel table-panel">
        ...
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    <?php if ($claims === []): ?>
                        <tr><td colspan="7">No active warranty cases found.</td></tr>
                    <?php endif; ?>
                    <?php foreach ($claims as $claim): ?>
                        ...
                        >
                            ...
                            <td>
                                <?php if ((int) ($claim['sale_id'] ?? 0) > 0): ?>
                                    <a class="icon-button" href="<?php echo e(app_url('?page=sale-view&id=' . (int) $claim['sale_id'])); ?>" aria-label="View invoice">
                                        <i data-lucide="eye"></i>
                                    </a>
                                <?php endif; ?>
                            </td>
    ...
    <div class="modal-card claim-update-modal" role="dialog" aria-modal="true" aria-labelledby="claim-update-title">
        <div class="panel-header">
            <div>
                ...
                <div class="claim-current-state" data-warranty-current-state hidden>
                    <span>Case <strong data-warranty-current-status></strong></span>
                    <span>Customer <strong data-warranty-current-customer></strong></span>
                    <span>Supplier <strong data-warranty-current-supplier></strong></span>
                </div>
            </div>
...
function wr_replacement_summary(array $claim): string
{
    ...
    $replacementMode = (string) ($claim['replacement_mode'] ?? '');

    if ($replacementMode === 'exchange') {
        $supplierText = match ($supplierStatus) {
            'received' => 'Supplier received',
            'refunded' => 'Supplier refund received',
            'none' => 'Shop loss',
            default => (string) ($claim['status'] ?? '') === 'sent_to_supplier'
                ? 'Supplier pending'
                : 'Supplier decision pending',
        };

        return 'Customer exchanged / ' . $supplierText;
    }
    ...
    if ($customerStatus === 'refunded') {
        return match ($supplierStatus) {
            'received' => 'Customer refunded / Supplier received',
            'refunded' => 'Customer refunded / Supplier refund received',
            'none' => 'Customer refunded / Shop loss',
            default => 'Customer refunded / Supplier pending',
        };
    }
