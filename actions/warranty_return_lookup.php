...
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    warranty_return_lookup_json(405, ['matches' => []]);
}

if (! $dbReady || $pdo === null) {
    warranty_return_lookup_json(503, ['matches' => []]);
}
...
try {
    ...
    warranty_return_lookup_json(500, ['matches' => []]);
}

function warranty_return_lookup_search(PDO $pdo, string $query): array
{
...
function warranty_return_lookup_invoices(PDO $pdo, int $customerId): array
{
    ...
    return array_map('warranty_return_lookup_invoice_payload', $statement->fetchAll());
}

function warranty_return_lookup_items(PDO $pdo, int $saleId): array
{
    if ($saleId <= 0) {
        return [];
    }
...
function warranty_return_lookup_invoice_payload(array $invoice): array
{
    return [
        'type' => 'invoice',
        ...
        'meta' => (string) $invoice['customer_name'] . ' / ' . date('Y-m-d H:i', strtotime((string) $invoice['sale_date'])) . ' / ' . (int) $invoice['available_units'] . ' item(s)',
    ];
}

function warranty_return_lookup_json(int $status, array $payload): never
{
